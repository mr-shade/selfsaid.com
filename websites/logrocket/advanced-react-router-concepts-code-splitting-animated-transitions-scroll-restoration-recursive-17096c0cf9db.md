---
title: "Advanced React Router concepts: Recursive path, code splitting, and more - LogRocket Blog"
date: "2017-11-08T03:00:28+00:00"
slug: "advanced-react-router-concepts-code-splitting-animated-transitions-scroll-restoration-recursive-17096c0cf9db"
image: "https://blog.logrocket.com/wp-content/uploads/2017/11/advanced-react-router-concepts.jpeg"
description: "Before setting down the path of advanced concepts, first let’s all agree on the basics. React Router provides: Routing capabilities […]"
tags: []
original_url: "https://blog.logrocket.com/advanced-react-router-concepts-code-splitting-animated-transitions-scroll-restoration-recursive-17096c0cf9db/"
---
...
![Advanced React Router Concepts](https://blog.logrocket.com/wp-content/uploads/2017/11/advanced-react-router-concepts.jpeg)
...
Check out the `code-splitting` branch and navigate to the `index.js` file in the routes folder (`/src/routes/index.js`). Or, you can view the file online [here](https://github.com/yomete/advanced-react-router-usage/blob/code-splitting/src/routes/index.js).
...
![](https://storage.googleapis.com/blog-images-backup/1*mJ4TD5rUUCEEC3TvY7Ev6g.png)
...
Animated transitions help to provide an easy flow to navigating a site. There are many React plugins that help with this in React but we’ll be considering the [react-router-transition](https://github.com/maisano/react-router-transition) plugin for the app.

Here is a preview of what we’ll be building:

![](https://storage.googleapis.com/blog-images-backup/1*mz1THW_Ne4JOtmYDK1B43A.gif)

Check out to the animated-transitions branch and navigate to the `index.js` file in the routes folder (`/src/routes/index.js`) or you can view the file online [here](https://github.com/yomete/advanced-react-router-usage/tree/animated-transitions). As seen above, I’ll only highlight the important bits of the code that helps with animated transitions.
...
Here is a [link](https://3r37589op6.codesandbox.io/) to see scroll restoration in action.

Let’s see how scroll restoration was implemented in the React app created above.

Check out to the `scroll-restoration` branch and navigate to the `index.js` file in the routes folder (`/src/routes/index.js`) or you can view the file online [here](https://github.com/yomete/advanced-react-router-usage/blob/scroll-restoration/src/routes/index.js).
...
Let’s open the `ScrollToTop` component to see the code for scroll restoration. Navigate to `src/components/ScrollToTop` and open the `ScrollToTop.js` or view the file online [here](https://github.com/yomete/advanced-react-router-usage/blob/scroll-restoration/src/components/ScrollToTop/ScrollToTop.js).
...
Check out to the `recursive-paths` branch and navigate to the `About.js` file in the About folder (`/src/components/About/About.js`) or you can view the file online [here](https://github.com/yomete/advanced-react-router-usage/blob/recursive-paths/src/components/About/About.js).
...
![](https://storage.googleapis.com/blog-images-backup/1*Ms4-PWaxL0QuDNoc7eJ5YQ.png)
...
The SSR React app is on the GitHub repo, you can check out the SSR branch or you can view the repo [here](https://github.com/yomete/advanced-react-router-usage/tree/server-side-rendering). I’ll highlight only the most important section of the app that touches on SSR.

The `webpack.development.config.js` file contains the webpack config needed for the React app and the contents of the file can be seen below or on [GitHub](https://github.com/yomete/advanced-react-router-usage/blob/server-side-rendering/webpack.development.config.js).
...
The entry point of the app is `server.js` which is the Node.js backend needed to run the app on a server. The content of the file can be seen below or on [GitHub](https://github.com/yomete/advanced-react-router-usage/blob/server-side-rendering/server.js).
...
app.get('/dist/main.css', (req, res) => {
  res.sendFile(path.join(\_\_dirname, '/public/main.css'))
});
...
In the code block above, we basically set up an Express web server in which the app will be run on and also set up a development server with `webpackDevMiddleware` and `webpackHotMiddleware`. At the top of the file, `requestHandler.js` is imported and this is used to build the app’s view later with `app.use(requestHandler)`. Let’s see the contents of that JavaScript file. You can also check it out [here](https://github.com/yomete/advanced-react-router-usage/blob/server-side-rendering/requestHandler.js).
...
The basic idea is that we wrap the app in a stateless `[<StaticRouter>](https://reacttraining.com/web/api/StaticRouter)` instead of a `[<BrowserRouter>](https://reacttraining.com/web/api/BrowserRouter)`. We then pass in the requested URL from the server so the routes can match and a context prop we’ll discuss next.
...
So how do we actually define the routes and the matching components in a server rendered app? That’s happening in the `src/router-config.js` [file](https://github.com/yomete/advanced-react-router-usage/blob/server-side-rendering/src/router-config.js) and the `src/components/App.js` [file](https://github.com/yomete/advanced-react-router-usage/blob/server-side-rendering/src/Components/App.js).
...
![](https://storage.googleapis.com/blog-images-backup/1*BMMIG2l3kMlXtd5-8WXWpQ.png)
...
![](https://storage.googleapis.com/blog-images-backup/1*PyOYgoASYN044Y_6fVDxCg.png)

## That’s it for now!

The codebase for this tutorial can be seen on this GitHub [repo](https://github.com/yomete/advanced-react-router-usage). There are different branches for each advanced concept. Feel free to browse through them and let me know what you think.
...
### Leave a Reply[Cancel reply](/advanced-react-router-concepts-code-splitting-animated-transitions-scroll-restoration-recursive-17096c0cf9db/#respond)
