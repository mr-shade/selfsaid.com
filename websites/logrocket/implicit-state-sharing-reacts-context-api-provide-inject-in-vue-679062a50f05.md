---
title: "Implicit state sharing: React’s context API & provide/inject in Vue - LogRocket Blog"
date: "2018-11-06T03:00:46+00:00"
slug: "implicit-state-sharing-reacts-context-api-provide-inject-in-vue-679062a50f05"
image: "https://blog.logrocket.com/wp-content/uploads/2018/11/implicit-state-sharing-nocdn.jpg"
description: "Imagine you are creating an accordion component that you want to distribute publicly through an npm package. You would like […]"
tags: []
original_url: "https://blog.logrocket.com/implicit-state-sharing-reacts-context-api-provide-inject-in-vue-679062a50f05/"
---
...
[![](https://blog.logrocket.com/wp-content/uploads/2019/04/jonathanharrell.jpg?w=150&h=150&crop=1)](https://blog.logrocket.com/author/jonathanharrell/)

[Jonathan Harrell](https://blog.logrocket.com/author/jonathanharrell/) UI/UX designer and frontend developer. Find me online at [jonathan-harrell.com](https://www.jonathan-harrell.com).
...
![Implicit State Sharing](https://blog.logrocket.com/wp-content/uploads/2018/11/implicit-state-sharing-nocdn.jpg)
...
Each `AccordionItem` will need to maintain some state — whether it is _expanded_ or not. But `AccordionHeader` will also need access to this value, so that it can show the appropriate toggle button. And `AccordionPanel` may also need to access this, since it is the thing being expanded and collapsed.
...
There is a better solution for cases like this — React’s Context API. We can use the Context API to create some state and provide it where needed behind the scenes, removing this concern from our public-facing API. (See also: [Compound Components pattern](https://blog.logrocket.com/guide-to-react-compound-components-9c4b3eb482e9)).
...
The `Provider` is one half of the Context equation. The other half is the `Consumer`. The `Provider` allows the `Consumer` to subscribe to context changes, as we will see soon.

Next, we need to set up `AccordionHeader` and `AccordionPanel` as _consumers_ of this context:
...
See the Pen[  
React Accordion Component Using Context](https://codepen.io/jonathanharrell/pen/WgBzyX) by Jonathan Harrell ([@jonathanharrell](https://codepen.io/jonathanharrell))  
...
See the Pen[  
Vue Accordion Component Using Provide/Inject](https://codepen.io/jonathanharrell/pen/PdvRoP) by Jonathan Harrell ([@jonathanharrell](https://codepen.io/jonathanharrell))  
on [CodePen](https://codepen.io).
...
### Leave a Reply[Cancel reply](/implicit-state-sharing-reacts-context-api-provide-inject-in-vue-679062a50f05/#respond)
